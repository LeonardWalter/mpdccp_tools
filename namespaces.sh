...
#iperfstr="iperf3 -c 192.168.102.10 -t 100 -4"
...
ns_setup () {
	...
	$ns1 bash -c 'client_setup "enp0s9" "100" "5"'
	$ns1 bash -c 'client_setup "enp0s10" "101" "4"'
...
[ "$1" == "-x" ] && reset
[ "$1" == "-p" ] && plot
[ -n "$ns1" ] && ns1="ip netns exec ns1"
[ "$(ip netns list | grep ns2)" ] || ns_setup
[ "$(ip netns exec ns2 ldt | grep tp0)" ] || tun_setup

$ns1 bash -c 'tc -s qdisc show dev enp0s9 && tc -s qdisc show dev enp0s10'
echo " "
echo "Testing $(</proc/sys/net/mpdccp/mpdccp_scheduler) scheduler enp9: $($ns1 cat /sys/module/mpdccplink/links/dev/enp0s9/mpdccp_prio) enp10: $($ns1 cat /sys/module/mpdccplink/links/dev/enp0s10/mpdccp_prio)"
